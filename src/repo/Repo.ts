...
export default class Repo<T extends IModel> {
  ...
  public createRecord = async (newData: Partial<T>): Promise<T> => {
    throw new Error(this.directCallError('createRecord'));
  }

  public getRecord = async (_id: string): Promise<T> => {
    throw new Error(this.directCallError('getRecord'));
  }

  public getAllRecords = async ({...args}): Promise<T[]> => {
    throw new Error(this.directCallError('getAllRecords'));
  }

  public updateRecord = async (_id: string, newData: Partial<T>): Promise<T> => {
